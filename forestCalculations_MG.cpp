#ifndef FORESTCALCULATIONS_CPP
#define FORESTCALCULATIONS_CPP

#ifdef unix
//#include "./dima/dima_MG.cpp"
#else
//#include "dima\dima.cpp"
#include "dima_MG.cpp"
#endif

const double coeffAgr = 2;

//******************************************************************************
// compressing data, sending them for further calculations
//******************************************************************************
vector<double> procPlots(g4m::dataStruct &plot, double minForest, int year,double OforestSharePrev, 
                          double AforestSharePrev,double deforRate_opt, double afforRate_opt,double hurdle_opt)
{
  double buf[47];
//********************************
//** compressing data to buffer **
//********************************  
  buf[0] =  minForest;
// from separate files

//  buf[1] =  data05x_BUILTUP[plot.x][plot.y][year];
//  buf[2] =  data05x_AREA[plot.x][plot.y][0];
//  buf[3] =  data05x_CROP[plot.x][plot.y][year];
//  buf[4] =  data05x_PRICEINDEX[plot.x][plot.y][year];
//  buf[5] =  data05x_POPDENS[plot.x][plot.y][year];
//  buf[6] =  data05x_DISCRATE[plot.x][plot.y][year];
//  buf[7] =  data05x_GDP[plot.x][plot.y][year];  

  buf[1] =  plot.BUILTUP[year];
  buf[2] =  plot.LANDAREA[0];
  buf[3] =  plot.CROP[year];
  buf[4] =  plot.PRICEINDEX[year];
  buf[5] =  plot.POPDENS[year];
  buf[6] =  plot.R[0];
  buf[7] =  plot.GDP[year];  

//cout<<"plot.POPDENS[year]: "<< buf[5]<<endl;

// from the combined file
  buf[8] =  plot.NPP[year];
  buf[9] =  plot.AGRSUIT[2000]; 
  buf[10] = plot.BIOMASS[year];
  buf[11] = plot.FOREST[year];
  buf[12] = plot.PROTECT[year];
// coefficients
  buf[13] = coeff.PriceIndexE[0];
  buf[14] = coeff.PriceLandMinR[0];
  buf[15] = coeff.PriceLandMaxR[0];
  buf[16] = coeff.FCuptake[0];
//  buf[17] = coeff.FTimber[0];
  buf[17] = plot.FTIMBER[year];
  buf[18] = coeff.HarvLoos[0];
  buf[19] = coeff.PriceC[0];
  buf[20] = coeff.FracLongProd[0];
  buf[21] = coeff.decRateL[0];
  buf[22] = coeff.decRateS[0];
  buf[23] = coeff.SlashBurn[0];
  buf[24] = coeff.FreqAid[0];
  buf[25] = coeff.PriceCAid[0];
  buf[26] = coeff.MaxRotInter[0];
  buf[27] = coeff.MinRotInter[0];
  buf[28] = coeff.baseline[0];
  buf[29] = coeff.PriceTimberMaxR[0];
  buf[30] = coeff.PriceTimberMinR[0];
  buf[31] = coeff.PlantingCostsR[0];
//  buf[32] = coeff.sPopDens[0]; // MG: corrected!!! 
  buf[32] = plot.SPOPDENS[year]; // MG: actual SPOPDENS must be used!
  buf[33] = coeff.PriceIndexE[0];
// model run options
  buf[34] = coeff.cellsInteract;
  buf[35] = coeff.inclAffor;
  buf[36] = coeff.noPay;
  buf[37] = coeff.uBiomass;
  buf[38] = coeff.litter;
  buf[39] = coeff.SOC;
// additional data  
  buf[40] = deforRate_opt;
  buf[41] = afforRate_opt;
  buf[42] = hurdle_opt;
  
  buf[43] = OforestSharePrev;
  buf[44] = AforestSharePrev; 
  
  buf[45] = plot.POTVEG[2000];
  buf[46] = plot.SAGRSUIT[2000];

//for (int i=13;i<43;i++){
//cout<<"buf["<<i<<"]= "<<buf[i]<<endl;}
//cout<<"SPOPDENS= "<<buf[32]<<endl;
//cout<<"GDP2000 = "<< (plot.GDP[2000]) << "\t"<< "GDP2005 = "<< (plot.GDP[2005]) << endl;

//cout<<"plot.NPP[year]: "<<plot.NPP[year]<<endl;
//cout<<"plot.POTVEG[0]: "<<plot.POTVEG[2000]<<endl;
  
//************************************
//** sending buffer and calculating **
//************************************
//  double res = calcPlots(buf,year);
  vector<double> res = calcPlots(buf,year);
  
  return (res);
}  

vector<double> calcPlots(double compr[],int year)
{
//************************
//** decompressing data **
//************************
  double minForest = compr[0];                 // minimum forest share among neighbouring cells
// from separate files
  double builtup       = compr[1];
  double landarea      = compr[2];
  double crop          = compr[3];
  double priceIndex    = compr[4];
  double popdens       = compr[5];
  double r             = compr[6];
  double GDPdat        = compr[7];
// from the combined file
  double NPP           = compr[8];
  double agrsuit       = compr[9];
  double biomass       = compr[10];
  double refForShare0  = compr[11];                                             // has to be calculated and cumulated every year
  double protect      = compr[12];
// coefficients
  double priceIndexE   = compr[13];
  double priceLandMinR = compr[14];
  double priceLandMaxR = compr[15];  
  double FCUptake      = compr[16];
  double ftimber       = compr[17];
  double harvloos      = compr[18];
  double pricec        = compr[19];
  double fraclongprod  = compr[20];
  double decratel      = compr[21];
  double decrates      = compr[22];
  double slashburn     = compr[23];
  double freqaid       = compr[24];
  double pricecaid     = compr[25];
// model run options
  int cellsInteract    = compr[34];
  int inclAffor        = compr[35];                                             // need to include code for afforestation
  int noPay            = compr[36];
  int uBiomass         = compr[37];                                             // need to include code for belowground biomass
  int litter           = compr[38];                                             // need to include code for belowground biomass
  int SOC              = compr[39];
  
  double dRate         = compr[40];
  double aRate         = compr[41];
  double hurdle_o      = compr[42];
  
  double OforestSharePrev  =compr[43];
  double AforestSharePrev  =compr[44];
  
  double potveg = compr[45];
  double sagrsuit     = compr[46];
    
                                               // need to include code for belowground biomass
//***************************
//** starting calculations **
//***************************
  vector<double> res;
  
  double aval, rotInter, priceTimber, forValNC;
  double OforestShare=OforestSharePrev;
  double AforestShare=AforestSharePrev;  
  double forestShare = OforestShare + AforestShare;  
  double refForShare = forestShare;            // forest share of ref. year
  
  double fsNoPay = forestShare;                              // Forest share without payment
  double fval = forVal(compr,rotInter,priceTimber,forValNC);
// calculating agricultural value with/without interaction
  if (!cellsInteract)
    aval = agrVal(compr);                               // without interaction
  else
    aval = coeffAgr * agrVal(compr) * (1 - minForest);  // with interaction

//if (protect == 0){

//cout<<"aval= "<<aval<<"\t"<<"fval= "<<fval<<endl;
//  double freeLand = 1 - (forestShare + crop + builtup);
  double  maxfor = 1. - (builtup + crop);
  		  if (maxfor < 0.) {maxfor = 0;}
  
// 0..Land has min Landprice, 1..Land has 2*minLandprice
//  double landvalue = aval / (priceLandMinR * priceIndex / priceIndexE) - 1;
// deforestation speed
  double defShare = 0;
  double affShare = 0;
   {
//cout << "popdens= " << popdens <<"\t"<<"agrsuit= " <<agrsuit<<"\t"<<"forestShare= "<<forestShare<<endl;
	double deflator = 0.8807; //8.807/10.; //MG: world GDP deflator to go from 2000 USD to 95 USD. i.e $10 2000 USD is worth $8.807 USD using a global price deflator. The source is the World Bank.
    double gdp = 1644.;
    if(popdens > 0){
      gdp = 200000 * GDPdat / popdens;
      gdp *= deflator;
      }
    if((agrsuit > 0) && (OforestShare > 0)) {
      defShare = 0.05/(1. + exp(1.799e+00 + 2.200e-01 / OforestShare + 1.663e-01 
                 / agrsuit + ( -4.029e-02 + 5.305e-04 * popdens) * popdens + 1.282e-04 * gdp));
	defShare *= dRate;//  3.52          //Corection to come to the defor rate of FAO-FRA

//cout<<"defRate= "<<dRate<<endl;

//cout<<"AGRSUIT= "<<agrsuit<<endl;
//cout<<"POPDENS= "<<popdens<<endl;
//cout<<"OforestShare= "<<OforestShare<<endl; 
//cout << "defShare=" << defShare << endl;

}
	

	    //MG: Afforestation
		  if(OforestShare + AforestShare < maxfor
           && potveg < 9 // MG: We afforest only places, where potential vegetation is forest
			  ) {
        affShare = 0.01/(1+exp(1.+0.1/agrsuit+1/(0.001*gdp)));
		if (affShare < 1./(landarea*1000000.)) {affShare = 0.;}   // minimun one tree (1m^2)
        affShare *= aRate; 
//cout << "affShare=" << affShare << endl;
//cout<<"affRate= "<<aRate<<endl;        
		  }
//cout << "potveg= " << potveg <<"\t"<<"maxfor= " <<maxfor<<endl;
	
	    } // MG: End of gdp definition

//// Wood consumption per capita
//  double woodConCapita = 5. /(1. + exp(2.221 - 2.242 * forestShare + 0.01227 * popdens -0.00005103 * 8500. * GDPdat));
//  double woodCon = woodConCapita * popdens * landarea;
//// Sustainable wood suply
//  double woodProd = 10. * NPP * FCUptake * landarea * 100. * forestShare * ftimber * (1. - harvloos);
//// Wood-suply deficit
//  double woodDef = woodProd - woodCon;
//  if (woodDef < 0.) {
//    woodDef *= -1.;
//	double addition = defShare * landarea * 100. * forestShare * biomass * ftimber;
//    if (woodDef > addition) woodDef = addition;
//    if (addition > 0) woodDef /= addition; else woodDef = 0.;
//  } else woodDef = 0.;

  double defIncome = 0.;
// Pay if Carbon get's to air (Harvest goes to products)
   {
    double pDefIncome = biomass * (priceTimber * ftimber
		              * (1. - harvloos) - pricec * (1 + r)
		              * (fraclongprod * decratel / (decratel + r)
		              + fraclongprod * decrates / (decrates + r)));
// Immediate Pay if deforested (Slash and Burn)
    double sDefIncome = biomass *
		                (priceTimber * ftimber
		              * (1. - harvloos)
		              - pricec);
    defIncome = pDefIncome * (1. - slashburn) + sDefIncome * slashburn;
   }
//cout<<"pricec= "<<pricec<<endl;
//cout<<"defIncome= "<<defIncome<<endl;

//cout<<"r = "<<r<<endl;

//  double defIncomeNoCp = 0.;  //Income with no carbon price
//  defIncomeNoCp = biomass * priceTimber * ftimber * (1. - harvloos);

//cout<<"biomass= "<<biomass<<"\t"<<"priceTimber= "<<priceTimber<<"\t"<<"ftimber= "<<ftimber<<"\t"<<"harvloos= "<<harvloos<<endl;

//// Aid
//  double aid = 0;
//  if ((biomass > 0) && (pricecaid > 0))
//   {
//    double tmpR = pow(r + 1, freqaid);
//    if (tmpR != 1.) aid = biomass * pricecaid * tmpR / (tmpR - 1.);
//   }
//  double protectionCosts = 0.;
//  if (r > .07) protectionCosts =(r-.07) * protcost * priceIndex / priceIndexE * (r + 1.) / r;
//  if ((aval + defIncomeNoCp) <= (fval * hurdle)) protectionCosts = 0.;
// Deforestation
//  if ((aval + defIncome) > (fval * hurdle )//+ aid - protectionCosts))
//  {
//    OforestShare -= defShare; //Decrease Forest share
////cout << "-defShare=" << defShare << endl;
//}
// Aforestation [HAS TO BE INCLUDED]
//Forest growth
//  double maxfor = 1. - (builtup + crop);
//  if (forestShare > maxfor) forestShare = maxfor;
//  if (forestShare < 0.) forestShare = 0.;
//  if (noPay) {
//Deforestation without Payment
//cout<<"hurdle= "<<hurdle<<endl;        

//cout<<"aval= "<<aval<<"\t"<<"defIncomeNoCp= "<<defIncomeNoCp<<"\t"<<"fVal= "<<fval<<"\t"<<"fValNC= "<<forValNC<<"\t"<<"hurdle_o= "<<hurdle_o<<"\t"<<"NPP= "<<NPP<<endl;
    if ((aval + defIncome) > (fval * hurdle_o ))// - protectionCosts)) && (NPP > 0.3)) 
    {
      OforestShare -= defShare;
      if (OforestShare > maxfor) OforestShare = maxfor;
      if (OforestShare < 0.) OforestShare = 0.;
    }
//cout<<"hurdle= "<< hurdle_o<<endl;  
  
  //Afforestation
  	    if(
	       (aval < fval * hurdle_o )	       
		   && (OforestShare + AforestShare < maxfor) //MG
//		   && data["POTVEG"].v()>0 
           && (potveg < 9) // MG: We afforest only places, where potential vegetation is forest
		) {
	    	   AforestShare += affShare;
//   	    if(AforestShare > maxfor) {AforestShare = maxfor;}
// cout << "affShare" << "\t" <<affShare << "\t" << "OforestShare + AforestShare" << "\t"<<OforestShare + AforestShare<<"\t"<< "AforestShare" << "\t"<<AforestShare<<endl;
	    if(AforestShare > maxfor) {AforestShare = maxfor;}
	    if(AforestShare < 0.) {AforestShare = 0.;}	    
		if(OforestShare + AforestShare > maxfor) {AforestShare = maxfor - OforestShare;}
	    if(AforestShare < 0.) {AforestShare = 0.;}
	       }
       
   	    if(OforestShare > maxfor) {OforestShare = maxfor;}
        if(OforestShare < 0.) {OforestShare = 0.;}	       
	    if(AforestShare < 0.) {AforestShare = 0.;}	       
	    if(AforestShare > maxfor) {AforestShare = maxfor;}	       
	    if(OforestShare + AforestShare > maxfor) {AforestShare = maxfor - OforestShare;}
        if(AforestShare < 0.) {AforestShare = 0.;}		    
        
		forestShare = OforestShare + AforestShare;
			
  
//  if(year <= 2000) {
//    refForShare = forestShare;
//    fsNoPay = forestShare;
//  }


//  double forestArea;
//  double forestBiomass;
//  double income;
//  double costs;
//  double costsb;
//  double costsc;
//  double forDec = refForShare - forestShare;
//  forestArea = forDec * landarea;
//  forestBiomass = forDec * landarea * 100. * biomass;
//  if (year > 2000) {
//    if (freqaid > 0.) {
//      costs = forestShare * landarea * 100. * biomass * pricecaid / freqaid;
//// Could we pay less and have the same result
//      if ((aval + defIncome) <= (fval * 1.5 + aid - protectionCosts) && (aval + defIncome) > (fval * 1.5)) {
//        double tmpR = pow(r+1, freqaid);
//        if((tmpR != 1.) && (freqaid > 0.) && (biomass > 0)) {
//          double pay = (aval + defIncome - fval * 1.5 + protectionCosts) / (biomass * tmpR / (r-1.));
//		  costsb = forestShare * landarea * 100. * biomass * pay / freqaid;
//// We don't pay for the whol grid, only for the saved forest
//          costsc = forestShare * landarea * 100. * biomass * pay / freqaid * (forestShare - fsNoPay);
//        }
//      }
//    }
//	if(forDec > 0.) income = forDec * landarea * 100. * biomass * pricec;
//  }

//} //MG: end if (protect ==0)

//cout<<"forestShare= "<<forestShare<<endl;
//cout<<"OforestShare= "<<OforestShare<<endl;
//cout<<"AforestShare= "<<AforestShare<<endl;

 res.push_back(forestShare);
 res.push_back(OforestShare);
 res.push_back(AforestShare);

 return(res);
//  return (forestShare);
}

vector<double> plotsData(g4m::dataStruct &plot, int year)
{
double data[19];
data[0] = plot.COUNTRY[2000];
data[1] = plot.POTVEG[2000];
data[2] = plot.PROTECT[2000];
data[3] = plot.USED[2000];
data[4] = plot.LANDAREA[2000];
data[5] = plot.NPP[year];
data[6] = plot.POPDENS[year];
data[7] = plot.SAGRSUIT[2000];
data[8] = plot.AGRSUIT[2000];
data[9] = plot.PRICEINDEX[year];
data[10] = plot.BIOMASS[year];
data[11] = plot.FOREST[2000];
data[12] = plot.R[2000];
data[13] = plot.GDP[year];
data[14] = plot.BUILTUP[year];
data[15] = plot.CROP[year];
data[16] = plot.FRACLONGPROD[2000];
data[17] = plot.CORRUPTION[2000];
data[18] = plot.SLASHBURN[2000];

vector<double> res;

res.push_back(data[0]);
res.push_back(data[1]);
res.push_back(data[2]);
res.push_back(data[3]);
res.push_back(data[4]);
res.push_back(data[5]);
res.push_back(data[6]);
res.push_back(data[7]);
res.push_back(data[8]);
res.push_back(data[9]);
res.push_back(data[10]);
res.push_back(data[11]);
res.push_back(data[12]);
res.push_back(data[13]);
res.push_back(data[14]);
res.push_back(data[15]);
res.push_back(data[16]);
res.push_back(data[17]);
res.push_back(data[18]);

return (res);
}

 

#endif
