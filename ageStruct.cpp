...
namespace g4m {
  ...
  ageStruct::v ageStruct::deforest(double area, int type) {
	...
	for(int i=0; i<activeAge; ++i) {
	  ...
	  ret.enSw += totalWood * sws.v(dat[i].d+id);
	  ret.enRw += totalWood * hle.v(dat[i].d+id) - totalWood * sws.v(dat[i].d+id);
	  ret.dbEn += totalWood * dbe.v(dat[i].d+id);
	  dat[i].area *= mul;
	}
      }
      ret.area=area;
  ...
  ageStruct::v ageStruct::finalCut(double area, bool eco) {
	...
	double id = it->gDi(i, mai, sdEt)/2.;
	//double ih = it->gH(i, mai)/2.;
	if(dbe.v(dat[i].d+id) * (dat[i].bm+dbm) >= minDbEn || eco==false) {
	  double totalWood = 0.;
	  ...
	  ret.enSw += totalWood * sws.v(dat[i].d+id);
	  ret.enRw +=totalWood * (hle.v(dat[i].d+id) - sws.v(dat[i].d+id));
	  ret.dbEn += totalWood * dbe.v(dat[i].d+id);
  ...
  ageStruct::v ageStruct::aging() {
	...
	dbm = tmp.first; thin = 0.;
      ...
      if(thin > 0. && dbv.v(dat[i].d+id) * thin >= minDbVn) {
	double sawnWood = sws.v(dat[i].d-id/2.); //Possible Sawn wood
	double restWood = hlv.v(dat[i].d-id/2.) - sawnWood;
	double totalWood = dat[i].area * thin;
	ret.vnSw += totalWood * sawnWood;
	ret.vnRw += totalWood * restWood;
	ret.dbVn += totalWood * dbv.v(dat[i].d-id/2);
      }
      dat[i+1] = dat[i];
    }
    double area=getArea();
...
// copy constructor
ageStruct::ageStruct(const ageStruct & g) {
    it = g.it;
    dat = g.dat;
    ageClasses = g.ageClasses;
    targetRotationPeriod = g.targetRotationPeriod;
    tragetHarvestArea = g.tragetHarvestArea;
    targetSd = g.targetSd;
    mai = g.mai;
    sws = g.sws;
    hlv = g.hlv;
